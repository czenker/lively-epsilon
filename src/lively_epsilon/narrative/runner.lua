...
Narrative.run = function(self, config)
    ...
    if not isEeStation(config.from) then error("Expected config.from to be a station, but got " .. typeInspect(config.from), 2) end
    if not isEeStation(config.to) then error("Expected config.to to be a station, but got " .. typeInspect(config.to), 2) end
