insulate("ShipTemplateBased", function()
    ...
    describe("withComms()", function()
        ...
        it("can set a hail text", function()
            ...
            assert.is_same(hail, station:getHailText(player))
        end)
        ...
        it("can set comms", function()
            ...
            assert.is_same(3, Util.size(station:getComms(player)))
        end)
    ...
    describe("setHailText(), getHailText()", function()
        local station = eeStationMock()
        ShipTemplateBased:withComms(station)

        it("returns nil if nothing else was set", function()
            assert.is_nil(station:getHailText(player))
        end)
        ...
        it("return nil if nil was set", function()
            ...
            assert.is_same(nil, station:getHailText(player))
        end)

        it("raises an error if getHailText() is called without argument", function()
            assert.has_error(function()
                station:getHailText()
            end)
        end)
        it("raises an error if getHailText() is called with a number", function()
            assert.has_error(function()
                station:getHailText(42)
            end)
        end)
    end)
    ...
    describe("addComms()", function()
        it("allows to be called with a reply", function()
            ...
            assert.is_same(1, Util.size(station:getComms(player)))
        end)
    ...
    describe("getComms()", function()
        it("returns all replies from the constructor and addComms()", function()
            ...
            assert.contains_value(reply1, comms)
            assert.contains_value(reply2, comms)
            assert.contains_value(reply3, comms)
        end)
        ...
        it("does not allow to manipulate internal state", function()
            ...
            table.insert(comms, commsScreenReplyMock())
